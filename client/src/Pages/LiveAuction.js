...
const LiveAuction = () => {
    ...
    const handleData = async () => {
        try {
            ...
            console.log('Response from productService:', response);  // Log response to check structure
            if (response) {
                setData(response);
            } else {
                message.error('Failed to load data');
            ...
            <div className="row">
                {
                    (!data || data.length === 0) ? (
                        ...
                        data?.map((auction, index) => (
                            <div key={auction._id} className="col-md-4 mb-4">
                                <div className="card">
                                    {/* Uncomment and adjust image source if necessary */}
                                    <img
                                        src={`${auction.product.image}`}
                                    ...
                                    <div className="card-body">
                                        ...
                                        <Link className="btn btn-primary" to={`/services/live-auction-bid/${auction._id}`}>
                                            Bid here
                                        </Link>
                                    </div>
                                </div>
